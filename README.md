...
For ease of processing, the pubs voted for are encoded into a JSON string and then written to a Database by the gbg_submit.php. A timestamp is also written to the DB along with the membership number of the given user, this is simply updated if the user re-submits.
